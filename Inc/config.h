...
#define INACTIVITY_TIMEOUT        88       // Minutes of not driving until poweroff. it is not very precise.
...
// ################################# VARIANT_NUNCHUK SETTINGS ############################
  ...
  */
  ...
  #define PRI_INPUT2              2, -1024, 0, 1024, 0     // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
    ...
    #define CONTROL_SERIAL_USART2 0       // left sensor board cable, disable if ADC or PPM is used! For Arduino control check the hoverSerial.ino
    #define FEEDBACK_SERIAL_USART2        // left sensor board cable, disable if ADC or PPM is used!
    #define AUX_INPUT1            1, -1000, 0, 1000, 0     // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
    #define AUX_INPUT2            1, -1000, 0, 1000, 0     // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
  #else
...
// ############################ VARIANT_HOVERCAR SETTINGS ############################
  ...
  #define CONTROL_SERIAL_USART3 0         // use Serial USART3 as input. Number indicates priority for dual-input. Disable DEBUG_SERIAL_USART3! For Arduino control check the hoverSerial.ino

  #define DUAL_INPUTS                       // ADC*(Primary) + Sideboard_R(Auxiliary). Uncomment this to use Dual-inputs
  #define PRI_INPUT1              2,  1000, 0, 2500, 0  // Pedal Brake        TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
  #define PRI_INPUT2              2, -1000, 0, 1000, 0  // Pedal Accel        TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
...
// ########################### UART SETIINGS ############################
  ...
  #define SERIAL_TIMEOUT          210//160                     // [-] Serial timeout duration for the received data. 160 ~= 0.8 sec. Calculation: 0.8 sec / 0.005 sec
#endif
#if defined(FEEDBACK_SERIAL_USART2) || defined(CONTROL_SERIAL_USART2) || defined(DEBUG_SERIAL_USART2) || defined(SIDEBOARD_SERIAL_USART2)
  #ifndef USART2_BAUD
