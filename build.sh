...
EXTRA_CFLAGS="-DDUAL_INPUTS -USIDEBOARD_SERIAL_USART3"
...
if VARIANT="$VARIANT" TARGET_CHIP="$TARGET_CHIP" CHIP="$TARGET_CHIP" EXTRA_CFLAGS="$EXTRA_CFLAGS" make BUILD_DIR="$BUILD_DIR" all 2>&1; then
    ...
    echo "生成预处理宏清单..."
        ...
        $CC_TOOL -dM -E \
            ...
            -DUSE_HAL_DRIVER -D"$GCC_TARGET_CHIP" -D"$GCC_CHIP_ALIAS" -D$VARIANT $EXTRA_CFLAGS \
            -include Inc/config.h -xc /dev/null > "$MACRO_FILE" 2>/dev/null || true
    echo "已生成宏清单: $MACRO_FILE"
    echo "---- 相关宏摘要 ----"
    grep -E "^#define (VARIANT_HOVERCAR|VARIANT_TRANSPOTTER|DUAL_INPUTS|CONTROL_ADC|CONTROL_SERIAL_USART3|CONTROL_SERIAL_USART2|FEEDBACK_SERIAL_USART3|FEEDBACK_SERIAL_USART2|SIDEBOARD_SERIAL_USART3|SIDEBOARD_SERIAL_USART2|PRI_INPUT1|PRI_INPUT2|AUX_INPUT1|AUX_INPUT2|USE_HAL_DRIVER|STM32F103xE|GD32E103RBT6|GD32F103RCT6)" "$MACRO_FILE" || true
    echo "--------------------"
else
    echo ""
    echo "编译失败！"
